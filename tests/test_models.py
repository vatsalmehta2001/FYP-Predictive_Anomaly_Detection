...
def create_synthetic_energy_series(n_points: int = 144, noise_level: float = 0.1) -> np.ndarray:
...
def create_forecasting_windows_synthetic(n_windows: int = 5) -> List[Dict]:
    ...
    for i in range(n_windows):
        ...
        target_times = base_time + np.arange(history_len, history_len + target_len) * np.timedelta64(30, 'm')
...
@pytest.mark.skipif(not torch.cuda.is_available() and os.getenv("CI"), reason="Skip GPU tests in CI")
class TestPatchTST:
    ...
    def test_patchtst_prediction_quantiles(self):
        ...
        try:
            ...
            quantile_forecasts = forecaster.predict(history, steps=8, return_quantiles=True)
...
class TestAutoencoder:
    ...
    def test_autoencoder_anomaly_detector(self):
        ...
        try:
            ...
            normal_sequences = [
                create_synthetic_energy_series(96, noise_level=0.05)
                for _ in range(3)
            ]
...
class TestConfig:
    ...
    def test_config_validation(self):
        ...
        from fyp.config import ExperimentConfig, ForecastingConfig, AnomalyConfig
...
class TestQuantileCoverage:
    ...
    def test_quantile_coverage_synthetic(self):
        ...
        q50 = np.percentile(true_values, 50) * np.ones(n_points)
...
class TestAnomalyLatency:
    ...
    def test_synthetic_spike_detection(self):
        ...
        if true_starts and predicted_times:
            latency_metrics = detection_latency(true_starts, predicted_times, max_delay=10)
