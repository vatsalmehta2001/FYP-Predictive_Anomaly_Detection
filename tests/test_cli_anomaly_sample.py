    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if (
                    "torch" in result.stderr.lower()
                    or "pytorch" in result.stderr.lower()
                ):
            ...
            created_files = list(output_dir.glob("*.csv")) + list(
            ...
            assert (
                len(created_files) >= 1
            ), f"Expected metrics files, found: {list(output_dir.iterdir())}"
            ...
            if len(created_plots) >= 1:
                ...
                assert (
                    plot_file.stat().st_size > 1000
                ), f"Plot file {plot_file} seems too small"

            print(
                f"[PASS] CLI test successful: {len(created_files)} files, {len(created_plots)} plots"
            )
...
def test_cli_anomaly_baseline_sample():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            assert (
                len(created_files) >= 1
            ), f"No output files created: {list(output_dir.iterdir()) if output_dir.exists() else 'directory does not exist'}"

            print(
                f"[PASS] Baseline CLI test successful: {len(created_files)} artifacts"
            )
...
def test_cli_help_text():
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=5, env=env, cwd=str(Path.cwd())
        )
