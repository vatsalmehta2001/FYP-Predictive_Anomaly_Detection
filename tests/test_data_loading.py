...
class TestSampleDataLoading:
    ...
    def test_load_lcl_sample(self):
        ...
        assert any(
            "time" in col or "date" in col for col in columns_lower
        ), "No timestamp column found"
        assert any(
            "kwh" in col or "energy" in col for col in columns_lower
...
class TestSSENMetadataLoading:
    ...
    def test_load_ssen_metadata(self):
        ...
        assert (
            df["voltage_tolerance_pct"].iloc[0] == 10.0
        ), "Incorrect voltage tolerance"
...
class TestDataValidation:
    ...
    def test_timestamp_alignment_30min(self):
        ...
        aligned = ts.replace(
            minute=0 if ts.minute < 30 else 30, second=0, microsecond=0
        )
...
def test_documentation_exists():
    ...
    assert (
        PROJECT_ROOT / "docs" / "anomaly_strategy.md"
    ).exists(), "anomaly_strategy.md missing"
...
def test_raw_data_exists():
    ...
    if ukdale_file.exists():
        assert (
            ukdale_file.stat().st_size > 1e9
        ), "UK-DALE file too small (should be ~6.3GB)"
