...
class TestUtils:
    ...
    def test_create_sliding_windows(self):
        ...
        windows = create_sliding_windows(data, context_length=336, forecast_horizon=48, stride=24)
    ...
    def test_normalize_denormalize(self):
        ...
        assert np.abs(np.mean(normalized)) < 1e-10  # Zero mean
        assert np.abs(np.std(normalized) - 1.0) < 1e-10  # Unit variance
...
class TestConstraints:
    ...
    def test_temporal_pattern_constraint(self):
        ...
        constraint = TemporalPatternConstraint(
            min_daily_consumption=2.0,
            max_daily_consumption=50.0
        )
...
class TestVerifierAgent:
    ...
    @pytest.fixture
    def temp_constraints_file(self):
    ...
    def test_verifier_initialization(self, temp_constraints_file):
        ...
        assert all(c in summary for c in ["non_negativity", "household_max", "ramp_rate"])
    ...
    def test_verifier_evaluate(self, temp_constraints_file):
        ...
        assert reward == 0.0  # Perfect compliance
...
class TestSolverAgent:
    ...
    def test_solver_with_scenario(self, solver, temp_constraints_file):
        ...
        # Predictions should be different with scenario
        assert not np.array_equal(
            predictions_baseline["0.5"],
            predictions_scenario["0.5"]
        )
...
class TestSelfPlayTrainer:
    ...
    def test_train_episode(self, trainer_setup):
        ...
        batch = [
            (np.random.rand(336), np.random.rand(16))
            for _ in range(4)
        ]
    ...
    def test_checkpoint_save_load(self, temp_constraints_file):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            new_solver = SolverAgent(
                model_config=solver.model_config,
                device="cpu",
                pretrain_epochs=0
            )
