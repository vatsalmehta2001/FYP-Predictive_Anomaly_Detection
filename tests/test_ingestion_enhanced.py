...
class TestDSTCorrectness:
    ...
    def test_spring_dst_transition(self):
        ...
        base_time = datetime(2023, 3, 26, 0, 0, tzinfo=timezone.utc)
        utc_timestamps = []
        ...
        assert len(unique_timestamps) == len(processed_timestamps), \
    ...
    def test_fall_dst_transition(self):
        ...
        base_time = datetime(2023, 10, 29, 0, 0, tzinfo=timezone.utc)
        timestamps = []
    ...
    def test_ensure_monotonic_timestamps(self):
        ...
        for entity in df_clean['entity_id'].unique():
            ...
            assert timestamps.is_monotonic_increasing, f"Non-monotonic for entity {entity}"
...
class TestDataQuality:
    ...
    def test_quality_metrics_calculation(self):
        ...
        problem_data = np.array([np.nan, np.nan, -0.5, 10.0, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan])
...
class TestDSTSpecific:
    ...
    def test_lcl_dst_spring_forward(self):
        ...
        for i in range(1, len(utc_timestamps)):
            assert utc_timestamps[i] > utc_timestamps[i-1], \
    ...
    def test_large_dataset_simulation(self):
        ...
        for i in range(min(n_readings, 1000)):  # Limit for test speed
            reading = EnergyReading(
                ...
                ts_utc=base_time.replace(minute=(i * 30) % 60, hour=((i * 30) // 60) % 24),
                interval_mins=30,
