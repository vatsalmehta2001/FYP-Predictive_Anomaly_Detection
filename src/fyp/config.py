...
class AnomalyConfig(BaseModel):
    ...
    hidden_sizes: list[int] = Field(default=[32, 16, 8], description="Hidden layer sizes")
    dropout: float = Field(default=0.1, description="Dropout rate")
    activation: str = Field(default="relu", description="Activation function")
    
    # Anomaly detection settings
    contamination: float = Field(default=0.05, description="Expected contamination rate")
...
class ExperimentConfig(BaseModel):
    ...
    entities: Optional[list[str]] = Field(default=None, description="Entity IDs to process")
    ...
    # Output settings
    output_dir: str = Field(default="data/derived/evaluation", description="Output directory")
    ...
    # MLflow settings
    mlflow_experiment: str = Field(default="energy_forecasting", description="MLflow experiment name")
