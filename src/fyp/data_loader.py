...
class EnergyDataLoader:
    ...
    def load_dataset(
        ...
        logger.info(
            f"Loaded {len(df)} records for {df['entity_id'].nunique()} entities"
        )
    ...
    def create_train_test_split(
        ...
        if split_by_time:
            ...
            test_df = (
                pd.concat(test_list, ignore_index=True) if test_list else pd.DataFrame()
            )
        else:
            # Random split
    ...
    def create_forecasting_windows(
        ...
        for entity_id in df["entity_id"].unique():
            ...
            for i in range(
                0, len(entity_df) - history_length - forecast_horizon + 1, step_size
            ):
                history = entity_df.iloc[i : i + history_length]
                future = entity_df.iloc[
                    i + history_length : i + history_length + forecast_horizon
                ]
