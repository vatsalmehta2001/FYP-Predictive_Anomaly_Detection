...
from .base import BaseIngestor, ensure_timezone_aware, round_to_interval
from .schema import EnergyReading
from .utils import convert_power_to_energy, calculate_data_quality_metrics, calculate_sha256
...
class UKDALEIngestor(BaseIngestor):
    ...
    def _create_downsampled_version(self) -> None:
        ...
        # Aggregate energy
        grouped = df.groupby(["entity_id", "ts_30min"]).agg({
            "energy_kwh": "sum",
            "source": "first",
            "extras": "first",
        }).reset_index()
