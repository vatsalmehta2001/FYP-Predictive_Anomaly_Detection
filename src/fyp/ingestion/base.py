...
class BaseIngestor(ABC):
    ...
    def validate_reading(self, reading: EnergyReading) -> str | None:
    ...
    def write_parquet_batch(
        self, readings: list[EnergyReading], output_dir: Path
    ) -> None:
        ...
        logger.info(
            f"Wrote batch {self._batch_counter}: {len(table)} records → {filename}"
        )
    ...
    def write_summary(self, output_dir: Path) -> None:
        ...
        if dataset_dir.exists():
            try:
                ...
                sample_df = (
                    pl.scan_parquet(str(dataset_dir / "*.parquet"))
                    .head(100000)
                    .collect()
                )
                if len(sample_df) > 0:
...
def ensure_timezone_aware(
    dt: pd.Timestamp, source_tz: str = "Europe/London"
) -> datetime:
