...
from fyp.selfplay.utils import compute_temporal_features
...
class HouseholdMaxConstraint(Constraint):
    ...
    def evaluate(
        ...
        typical_violations = np.sum((forecast > self.typical_max) & (forecast <= self.absolute_max))
...
class VoltageConstraint(Constraint):
    ...
    def __init__(self, nominal_voltage: float = 230.0, tolerance: Tuple[float, float] = (-0.06, 0.10)):
...
class VerifierAgent:
    ...
    def __init__(self, ssen_constraints_path: str):
        ...
        logger.info(f"Initialized VerifierAgent with {len(self.constraints)} constraints")
    ...
    def _initialize_constraints(self) -> Dict[str, Constraint]:
        ...
        constraints = {
            ...
            "household_max": HouseholdMaxConstraint(
                ...
                absolute_max_kwh=ssen_data["household_limits"]["absolute_max_kwh_30min"]
            ),
