...
class SolverAgent:
    ...
    def __init__(
        ...
        if PatchTSTForecaster is not None:
            ...
            self.model = None
            logger.warning("PatchTSTForecaster not available. Using fallback prediction methods.")
    ...
    def _pretrain_on_historical_data(
        ...
        """
        logger.info(f"Pretraining solver on historical data for {pretrain_epochs} epochs")
        ...
        try:
            ...
            if self.use_samples:
                ...
                windows = []
                for household_data in train_data[:100]:  # Limit households for efficiency
                    ...
                    for context, target in household_windows[:10]:  # Limit windows per household
                        windows.append({
    ...
    def predict(
        ...
        if self.model.model is None:
            # Model not trained yet, return naive forecast
        ...
        if scenario is not None:
            ...
            modified_context = np.concatenate([
                context_window[:-len(scenario_hint)],
                scenario_hint
            ])
        else:
    ...
    def train_step(
        ...
        if scenario is not None:
            ...
            self.scenario_adaptation_weights[scenario_key] *= (1.0 + verification_reward * 0.1)
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        verification_penalties = [m["verification_penalty"] for m in self.training_metrics]
