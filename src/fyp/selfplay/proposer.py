...
from fyp.selfplay.verifier import Constraint
...
class ProposerAgent:
    ...
    def propose_scenario(
        ...
        conditioning_samples: Optional[List[Tuple[ScenarioProposal, float]]] = None,
    ...
    def _select_scenario_type(
        self, 
        conditioning_samples: Optional[List[Tuple[ScenarioProposal, float]]]
    ) -> str:
        ...
        if conditioning_samples:
            recent_types = [s[0].scenario_type for s in conditioning_samples[-10:]]
    ...
    def _validate_physics_constraints(self, proposal: ScenarioProposal) -> bool:
        ...
        if np.any(test_forecast > self.constraints["household_limits"]["absolute_max_kwh_30min"]):
            return False
        ...
        if proposal.scenario_type == "EV_SPIKE":
            ...
            max_ev_power = self.constraints.get("ev_charging", {}).get("fast_charge_power_kw", 7.0)
    ...
    def _estimate_difficulty(
        ...
        if len(proposal.baseline_context) > 1:
            volatility = np.std(proposal.baseline_context) / (np.mean(proposal.baseline_context) + 1e-6)
        else:
            volatility = 0.1
    ...
    def _apply_curriculum_adjustment(self, proposal: ScenarioProposal) -> ScenarioProposal:
