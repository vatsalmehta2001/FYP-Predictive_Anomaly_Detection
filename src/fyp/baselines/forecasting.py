...
class BaseForecaster(ABC):
    ...
    @abstractmethod
    def fit(self, history: np.ndarray, timestamps: Optional[np.ndarray] = None) -> None:
...
class LinearTrendForecaster(BaseForecaster):
    ...
    def _create_features(
        ...
        if len(values) >= 7 * self.seasonal_period:
            ...
            day_sin = np.sin(2 * np.pi * day_pattern / (7 * self.seasonal_period)).reshape(-1, 1)
            day_cos = np.cos(2 * np.pi * day_pattern / (7 * self.seasonal_period)).reshape(-1, 1)
...
class EnsembleForecaster(BaseForecaster):
    ...
    def __init__(self, forecasters: list, weights: Optional[list] = None):
