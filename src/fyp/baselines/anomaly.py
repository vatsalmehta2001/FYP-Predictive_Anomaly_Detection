...
class BaseAnomalyDetector(ABC):
    ...
    @abstractmethod
    def fit(self, data: np.ndarray, timestamps: Optional[np.ndarray] = None) -> None:
...
class DecompositionAnomalyDetector(BaseAnomalyDetector):
    ...
    def _decompose_simple(self, data: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def predict_scores(self, data: np.ndarray) -> np.ndarray:
        ...
        if self.residual_stats["std"] > 0:
            ...
            scores = np.abs(residual - self.residual_stats["mean"]) / self.residual_stats["std"]
        else:
        ...
        for i in range(n):
            ...
            if len(window_data) >= 5:  # Minimum window
                ...
                if self.baseline_stats["std"] > 0:
                    z_score = abs(data[i] - self.baseline_stats["mean"]) / self.baseline_stats["std"]
                    scores[i] = z_score / self.n_sigma
