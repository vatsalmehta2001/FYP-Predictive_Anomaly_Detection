...
class EnergyPatchDataset(Dataset):
    ...
    def _create_patches(self) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        ...
        for seq, target in zip(self.sequences, self.targets):
...
class PatchTSTForecaster:
    ...
    def fit(self, windows: List[Dict], validation_split: float = 0.2) -> Dict:
        ...
        # Normalize data
        all_data = np.concatenate([np.concatenate(sequences), np.concatenate(targets)])
        normalized_sequences = [self._normalize_data(seq, fit=(i == 0)) for i, seq in enumerate(sequences)]
        ...
        train_dataset = EnergyPatchDataset(
            ...
            self.config["patch_len"], stride=self.config["patch_len"] // 2
        )
        
        if val_sequences:
            val_dataset = EnergyPatchDataset(
                val_sequences, val_targets,
                self.config["patch_len"], stride=self.config["patch_len"]
            )
        else:
            val_dataset = None
    ...
    def predict(
        ...
        dataset = EnergyPatchDataset(
            [normalized_history], [np.zeros(steps)],  # Dummy target
            self.config["patch_len"], stride=self.config["patch_len"]
        )
        ...
        if len(dataset) == 0:
            ...
            if return_quantiles:
                return {str(q): median_forecast.copy() for q in self.config["quantiles"]}
            else:
